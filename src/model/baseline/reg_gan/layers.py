...
def get_init_function(activation, init_function, **kwargs):
    ...
    if isinstance(init_function, str):
        if init_function == "kaiming":
            ...
            return partial(
                torch.nn.init.kaiming_normal_, a=a, nonlinearity=activation, mode="fan_in"
            )
...
class Conv(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.resnet_block = (
            ResnetTransformer(out_channels, resnet_n_blocks, init_func) if use_resnet else None
        )
        ...
        **kwargs
        ...
        if self.use_attention:
            self.attention_gate = AttentionGate(
                nc_down_stream, nc_skip_stream, nc_inner, use_norm=True, init_func=init_func
            )
        self.up_conv = Conv(
            nc_inner,
            nc_out,
...
class AttentionGate(torch.nn.Module):
    def __init__(
        self, nc_g, nc_x, nc_inner, use_norm=False, init_func="kaiming", mask_channel_wise=False
    ):
    ...
    def forward(self, g, x):
        ...
        if x_c.size(2) != g_size[2] and x_c.size(3) != g_size[3]:
            x_c = F.interpolate(
                x_c, (g_size[2], g_size[3]), mode=up_sample_mode, align_corners=align_corners
            )
        ...
        if alpha_size[2] != x_size[2] and alpha_size[3] != x_size[3]:
            alpha = F.interpolate(
                x, (x_size[2], x_size[3]), mode=up_sample_mode, align_corners=align_corners
            )
        return alpha * x
