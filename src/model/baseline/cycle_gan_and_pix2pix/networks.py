...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(
        ...
        if outermost:
            ...
            if final_activation is None:
                raise ValueError("Final activation must be given for the outermost U-Net block!")
            if final_activation == "tanh":
                up += [nn.Tanh()]
            elif final_activation != "linear":
                raise ValueError("Unknown final activation!")
    ...
    @staticmethod
    def _pad_if_not_even(tensor):
        if tensor.shape[3] % 2 != 0:
            left_right_padding = (0, 1)
            crop_left_right = (slice(-1),)
        else:
            left_right_padding = (0, 0)
            crop_left_right = (slice(None),)
        if tensor.shape[2] % 2 != 0:
            top_bottom_padding = (0, 1)
            crop_top_bottom = (slice(-1),)
        else:
            top_bottom_padding = (0, 0)
            crop_top_bottom = (slice(None),)
        padding = left_right_padding + top_bottom_padding
        padded = torch.nn.functional.pad(tensor, padding)
        crop = (...,) + crop_top_bottom + crop_left_right
        return padded, crop

    def forward(self, x):
        x_padded, crop = self._pad_if_not_even(x)
        output = self.model(x_padded)[crop]
        if self.outermost:
            return output
