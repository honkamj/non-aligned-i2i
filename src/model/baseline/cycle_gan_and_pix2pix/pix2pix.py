...
class Pix2PixModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.mask = input["A_mask"].detach() * input["B_mask"].detach()
        self.real_A = input["A" if AtoB else "B"].to(self.device)
        self.real_B = input["B" if AtoB else "A"].to(self.device)
        if self.use_masking:
            self.real_A = self.real_A * self.mask
            self.real_B = self.real_B * self.mask
    ...
    def state_dict(self):
        ...
        state_dictionary = {"netG": netG.state_dict()}
        if self.isTrain:
