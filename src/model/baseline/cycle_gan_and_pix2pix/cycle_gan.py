"""Mostly copied from https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix/"""
...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.use_masking = getattr(opt, 'use_masking', True)
    ...
    def forward(self):
        ...
        if self.use_masking:
            self.fake_B = self.fake_B * self.mask_A
            self.rec_A = self.rec_A * self.mask_A
            self.fake_A = self.fake_A * self.mask_B
            self.rec_B = self.rec_B * self.mask_B

    def translate_A_to_B(self, image):
        """For inference A to B"""
        return self.netG_A(image)

    def translate_B_to_A(self, image):
        """For inference B to A"""
    ...
    def state_dict(self):
        netG_A = (
            self.netG_A.module if isinstance(self.netG_A, torch.nn.DataParallel) else self.netG_A
        )
        netG_B = (
            self.netG_B.module if isinstance(self.netG_B, torch.nn.DataParallel) else self.netG_B
        )
        state_dictionary = {"netG_A": netG_A.state_dict(), "netG_B": netG_B.state_dict()}
        if self.isTrain:
            netD_A = (
                self.netD_A.module
                if isinstance(self.netD_A, torch.nn.DataParallel)
                else self.netD_A
            )
            netD_B = (
                self.netD_B.module
                if isinstance(self.netD_B, torch.nn.DataParallel)
                else self.netD_B
            )
    ...
    def load_state_dict(self, state_dict):
        ...
        netG_B.load_state_dict(state_dict["netG_B"])
        if self.isTrain:
            netD_A = (
                self.netD_A.module
                if isinstance(self.netD_A, torch.nn.DataParallel)
            ...
            netD_A.load_state_dict(state_dict["netD_A"])
            netD_B.load_state_dict(state_dict["netD_B"])
