"""Training script for training combined registration and I2I with unconditional
discriminator"""
    ...
    for epoch in epoch_tqdm:
        ...
        try:
            for (input_image, label_image, input_mask, label_mask, random_deformation) in data_tqdm:
                ...
                combined_mask_discriminator = MaskedVolume.get_comparison_mask(label, output)
                discriminator_output_real = discriminator(
                    input_volume=None,
                    output_volume=label.volume.detach() * combined_mask_discriminator,
                )
                ...
                discriminator_output_fake = discriminator(
                    input_volume=None,
                    output_volume=output.volume.detach() * combined_mask_discriminator,
                )
                ...
                discriminator_output_generator_fake = discriminator(
                    input_volume=None, output_volume=output.volume * combined_mask_discriminator
                )
