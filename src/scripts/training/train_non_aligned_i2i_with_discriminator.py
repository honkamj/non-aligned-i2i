"""Training script for training combined registration and I2I with
discriminator"""
...
from torch import device, float32, full
...
from model.interface import init_non_aligned_i2i_model, init_spectral_norm_discriminator
...
def _train(
    ...
    optimizer_generator = Adam(
        params=non_aligned_i2i.parameters(),
    ...
    use_eqadv = loss_config.get("use_eqadv", True)
    ...
    forward_rigidity_weight = loss_config.get("forward_rigidity_weight")
    inverse_rigidity_weight = loss_config.get("inverse_rigidity_weight")
    self_di_weight = loss_config.get("self_di_weight")
        ...
        try:
            for (
                ...
                (
                    ...
                    label_for_discriminator,
                    output_for_discriminator,
                ) = non_aligned_i2i(
                    ...
                    forward_random_transformation=random_deformation[:half_batch_size],
                    desired_outputs=(
                        "random_deformed_input",
                        "random_cross_modality_deformed_label"
                        if use_eqadv
                        else "cross_modality_deformed_label",
                        "random_commutation_deformed_output" if use_eqadv else "deformed_output",
                    ),
                )
                label_real = full(
                    (half_batch_size,),
                ...
                discriminator_output_real = discriminator(
                    input_volume=(random_deformed_input.volume.detach()),
                    output_volume=(
                        label_for_discriminator.volume.detach() * combined_mask_discriminator
                    ),
                )
                ...
                discriminator_output_fake = discriminator(
                    input_volume=(random_deformed_input.volume.detach()),
                    output_volume=(
                        output_for_discriminator.volume.detach() * combined_mask_discriminator
                    ),
                )
                discriminator_error_fake = calculate_discriminator_loss(
                    discriminator_output_fake, label_fake
                )
                ...
                (
                    ...
                    rigid_angles,
                    rigid_translations,
                    predicted_deformation_left_deformed_output,
                    predicted_deformation_right_deformed_output,
                ) = non_aligned_i2i(
                    input_volume=input_image[half_batch_size:],
                    label_volume=label_image[half_batch_size:],
                    input_mask=input_mask[half_batch_size:],
                ...
                discriminator_output_generator_fake = discriminator(
                    input_volume=(random_deformed_input.volume),
                    output_volume=(output_for_discriminator.volume * combined_mask_discriminator),
                )
                discriminator_error_generator = calculate_discriminator_loss(
                    discriminator_output_generator_fake, label_real
                )
                ...
                self_di_loss = (
                    calculate_output_similarity_loss(
                        predicted_deformation_left_deformed_output.volume,
                        predicted_deformation_right_deformed_output.volume,
                        MaskedVolume.get_comparison_mask(
                            predicted_deformation_left_deformed_output,
                            predicted_deformation_right_deformed_output,
                        ),
                ...
                forward_rigidity_loss = (
                    ...
                    if forward_rigidity_weight is not None
                    else 0.0
                )
                inverse_rigidity_loss = (
                    ...
                    if inverse_rigidity_weight is not None
                    else 0.0
                )
                ...
                intra_modality_deformation_mean_displacement = torch_mean(
                    sqrt(
                        torch_sum(
                            square(forward_intra_modality_deformation.get_deformation()),
                            dim=1,
                        )
                    )
                )
                ...
                generator_loss = (
                    ...
                    * intra_modality_similarity_loss
                    ...
                    + (forward_rigidity_weight if forward_rigidity_weight is not None else 0.0)
                    * forward_rigidity_loss
                    * 0.5
                    + (inverse_rigidity_weight if inverse_rigidity_weight is not None else 0.0)
                    * inverse_rigidity_loss
                    * 0.5
                    ...
                    + (self_di_weight if self_di_weight is not None else 0.0) * self_di_loss
                )
                loss_averager.count(
                    {
                        ...
                        "self_di": float(self_di_loss),  # type: ignore
                    }
                )
