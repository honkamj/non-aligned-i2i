...
Implementations are based on nitorch by Yael Balbastre and Mikael Brudfors.
...
@script
def _move_axis(input_tensor: Tensor, source: int, destination: int) -> Tensor:
...
def _transposed_conv1d(
    ...
    input_tensor = _move_axis(input_tensor, dim, -1)
    ...
    return _move_axis(convolved, -1, dim)
...
def _bound_causal(coefficients: Tensor, pole: float, dim: int = 0) -> Tensor:
    ...
    output = matmul(
        _move_axis(coefficients, dim, -1).unsqueeze(-2), pole_pows.unsqueeze(-1)
    ).squeeze(-1)
...
def _bound_anticausal(coefficients: Tensor, pole: float, dim: int = 0) -> Tensor:
    output = _move_axis(coefficients, dim, 0)[-1] * (pole / (pole - 1))
    return output
...
def _cubic_spline_coefficients_1d(coefficients: Tensor, dim: int = -1) -> Tensor:
    ...
    coefficients *= gain
    coefficients = _move_axis(coefficients, dim, 0)
    dim_size = coefficients.size(0)
    ...
    output = _move_axis(coefficients, 0, dim)
    return output
